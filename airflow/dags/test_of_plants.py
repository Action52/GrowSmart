...
dag = DAG('Plants_test', default_args=default_args, schedule_interval=None)

#create global variables for buckets
s3 = boto3.client('s3')
source_bucket = 'growsmartplanttraits'
...
def check_documents_existence():
    ...
    response = s3.list_objects(Bucket=source_bucket, Prefix='Species_mean_traits_testtest')
...
def extract_and_transform():
    ...
    pq.write_to_dataset(
        table=table,
        root_path=f"s3://{destination_bucket}/plant_traits_test/{current_date}",
