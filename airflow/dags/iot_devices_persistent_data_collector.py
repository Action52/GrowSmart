...
convert_and_move_to_parquet_task = PythonOperator(
    ...
    op_kwargs={
        ...
        "input_key": f'iot_data_{datetime.today().strftime("%Y-%m-%d")}/iot_data.csv',
        "output_key": f'iot_data_{datetime.today().strftime("%Y-%m-%d")}/iot_data.parquet',
    },
    dag=dag,
)
...
verify_csv_task = PythonOperator(
    ...
    op_kwargs={
        "bucket_name": source_bucket,
        "key_name": f'iot_data_{datetime.today().strftime("%Y-%m-%d")}/iot_data.csv',
    },
    dag=dag,
)
