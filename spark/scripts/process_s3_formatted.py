...
def create_garden_nodes_df(df: DataFrame):
    gardens = (
        df.select("iot_garden_id", "iot_garden_name", "weather_city", "iot_plant_id", "iot_sensor_id")
        .distinct()
...
def create_sensor_nodes_df(df: DataFrame):
    sensors = (
        df.select("iot_sensor_id", "iot_garden_id", "iot_plant_id", "event_id")
        .distinct()
...
def create_sensor_data_individual_nodes_df(df: DataFrame, sensor_data_cols=[]):
    ...
    sensor_data = df.select(all_cols).distinct()
    ...
    sensor_data = sensor_data.select(
        "iot_sensor_id",
        "iot_garden_id",
        "iot_plant_id",
        "event_id",
        expr(
            "stack("
            + str(len(sensor_data_cols))
            + ", "
            + ", ".join(["'" + c + "', " + c for c in sensor_data_cols])
            + ")"
        ).alias(":LABEL", "value"),
...
def create_event_nodes_df(df: DataFrame):
    ...
    events = df.select(all_cols).distinct().withColumn(":ID", col("event_id")).withColumn(":LABEL", lit("Event"))
    return events


def create_plant_nodes_df(df: DataFrame):
    plants = (
        df.select("iot_plant_id", "iot_garden_id", "iot_species_id", "iot_sensor_id")
        .distinct()
...
def create_location_nodes_df(df: DataFrame):
    locations = (
        df.select("weather_city", "iot_garden_id")
        .distinct()
...
def create_weather_individual_nodes_df(df: DataFrame, weather_cols=[]):
    ...
    weathers = df.select(
        all_cols
    ).distinct()  # .withColumn(":ID", generate_uuid(lit("WEATHER"), col("weather_city"), col("today"))) \
    # .withColumn(":LABEL", lit("WeatherForecast"))

    weathers = weathers.select([col(c).cast("string") for c in weathers.columns])
    # Use stack to pivot the dataframe from wide to long format
    weathers = weathers.select(
        "weather_city",
        "today",
        "weather_prediction_date",
        expr(
            "stack(" + str(len(weather_cols)) + ", " + ", ".join(["'" + c + "', " + c for c in weather_cols]) + ")"
        ).alias(":LABEL", "value"),
...
def create_date_nodes_df(df: DataFrame):
    dates_predicted = (
        df.select("weather_prediction_date", "event_id")
        .withColumn(":ID", col("weather_prediction_date"))
        .withColumn(":LABEL", lit("Date"))
        .select(":ID", ":LABEL")
    )
    dates_today = (
        df.select("today", "event_id")
...
def create_plant_characteristics_individual_nodes_df(df: DataFrame, plant_data_cols=[]):
    ...
    plant_data = df.select(all_cols).distinct()

    plant_data = plant_data.select([col(c).cast("string") for c in plant_data.columns])

    # Use stack to pivot the dataframe from wide to long format
    plant_data = plant_data.select(
        "iot_species_id",
        expr(
            "stack("
            + str(len(plant_data_cols))
            + ", "
            + ", ".join(["'" + c + "', " + c for c in plant_data_cols])
            + ")"
        ).alias(":LABEL", "value"),
...
def create_described_by_edge(species_df: DataFrame, plant_characteristics_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(species_df[":ID"].alias(":START_ID"), plant_characteristics_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_plant_type_edge(plant_df: DataFrame, species_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(plant_df[":ID"].alias(":START_ID"), species_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_measures_edge(plant_df: DataFrame, sensor_data_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(sensor_data_df[":ID"].alias(":START_ID"), plant_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_has_plant_edge(garden_df: DataFrame, plant_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(garden_df[":ID"].alias(":START_ID"), plant_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_has_sensor_edge(garden_df: DataFrame, sensor_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(garden_df[":ID"].alias(":START_ID"), sensor_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_registers_edge(sensor_df: DataFrame, event_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(sensor_df[":ID"].alias(":START_ID"), event_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_located_in_edge(garden_df: DataFrame, location_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(garden_df[":ID"].alias(":START_ID"), location_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_predicted_for_edge(weather_df: DataFrame, location_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(weather_df[":ID"].alias(":START_ID"), location_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_tomorrow_date_edge(weather_df: DataFrame, date_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(weather_df[":ID"].alias(":START_ID"), date_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_predicted_data_edge(weather_df: DataFrame, weather_data_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(weather_df[":ID"].alias(":START_ID"), weather_data_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_today_date_edge(event_df: DataFrame, date_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(event_df[":ID"].alias(":START_ID"), date_df[":ID"].alias(":END_ID"))
        .distinct()
...
def create_contains_edge(event_df: DataFrame, sensor_data_df: DataFrame):
    ...
    # Define the edges
    edges = (
        df.select(event_df[":ID"].alias(":START_ID"), sensor_data_df[":ID"].alias(":END_ID"))
        .distinct()
...
def transform(s3_master_url, s3_url, file_path, aws_access_key_id, aws_secret_access_key, mode="INFO"):
    ...
    df = (
        ...
        .withColumn("event_id", generate_uuid(lit("EVENT"), col("iot_sensor_id"), col("today"), col("iot_plant_id")))
    )
    ...
    sensor_data_nodes_df = sensor_data_nodes_df.drop(
        "iot_sensor_id", "iot_garden_id", "iot_plant_id", "event_id"
    ).distinct()
    ...
    for edge_df, name in zip(
        [
            described_by_edge_df,
            plant_type_edge_df,
            measures_edge_df,
            has_plant_edge_df,
            has_sensor_edge_df,
            registers_edge_df,
            located_in_edge_df,
            predicted_for_edge_df,
            tomorrow_date_edge_df,
            today_date_edge_df,
            contains_edge_df,
            predicted_data_edge_df,
        ],
        [
            "described_by_edges",
            "plant_type_edges",
