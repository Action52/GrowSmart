...
class Spark:
    ...
    def _init_session(self):
        spark = (
            SparkSession.builder.appName("Kafka batch query")
...
class SparkStreaming(Spark):
    ...
    def init_streaming(self):
        ...
        df = (
            self.sc.readStream.format("kafka")
...
class StreamAnalytics:
    ...
    def _update_cms(self, rows, batch_id):
        ...
        for garden_id, count in self.heavy_hitters.items():
            document = {"garden_id": garden_id, "count": count}
    ...
    def count_stream(self):
        ...
        query = (
            ...
            .select(
                "value.*", to_timestamp(lit(F.col("value.timestamp")), "yyyy-MM-dd_HH-mm-ss").alias("parsed_timestamp")
            )
    ...
    def sensor_analytics(self):
        ...
        soil_temp = non_grouped_query.groupBy(
            F.window("parsed_timestamp", self.WINDOW_SIZE, self.SLIDE_DURATION), "species_id"
        ).agg(avg("soil_temp").alias("average_soil_temp"))
        soil_humidity = non_grouped_query.groupBy(
            F.window("parsed_timestamp", self.WINDOW_SIZE, self.SLIDE_DURATION), "species_id"
        ).agg(avg("soil_humidity").alias("average_soil_humidity"))
        soil_nitrogen = non_grouped_query.groupBy(
            F.window("parsed_timestamp", self.WINDOW_SIZE, self.SLIDE_DURATION), "species_id"
        ).agg(avg("soil_nitrogen").alias("average_soil_nitrogen"))
        soil_potassium = non_grouped_query.groupBy(
            F.window("parsed_timestamp", self.WINDOW_SIZE, self.SLIDE_DURATION), "species_id"
        ).agg(avg("soil_potassium").alias("average_soil_potassium"))
        soil_phosporous = non_grouped_query.groupBy(
            F.window("parsed_timestamp", self.WINDOW_SIZE, self.SLIDE_DURATION), "species_id"
        ).agg(avg("soil_phosporous").alias("average_soil_phosporous"))
        soil_ph = non_grouped_query.groupBy(
            F.window("parsed_timestamp", self.WINDOW_SIZE, self.SLIDE_DURATION), "species_id"
        ).agg(avg("soil_ph").alias("average_soil_ph"))

        # Write to the elastic search
        temp_query = temp.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()

        co_query = co.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()

        humidity_query = humidity.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()

        smoke_query = smoke.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()

        lpg_query = lpg.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()

        rainfall_query = rainfall.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()

        soil_temp_query = soil_temp.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()

        soil_humidity_query = (
            soil_humidity.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()
        )

        soil_nitrogen_query = (
            soil_nitrogen.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()
        )

        soil_potassium_query = (
            soil_potassium.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()
        )

        soil_phosporous_query = (
            soil_phosporous.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()
        )

        soil_ph_query = soil_ph.writeStream.outputMode("update").foreachBatch(self._write_to_elasticsearch).start()
